    ...
    <div class="subcontent">
        ...
        <form class="stdform stdform2" method="post" action="@if(isset($objNews)) {{URL::action('NewsController@postUpdateNews')}} @else {{URL::action('NewsController@postAddNews')}}@endif">
            ...
            <p>
                ...
                <span class="field"><input type="text" name="newstitle" placeholder="Nhập tiêu đề bài viết" value="@if(isset($objNews)){{$objNews->newsName}}@endif" class="longinput"></span>
            </p>
            ...
            <p>
                ...
                <span class="field"><input type="text" name="newsKeywords" placeholder="Nhập từ khóa cho bài viết eg:Hình sự,Tin mới" value="@if(isset($objNews)){{$objNews->newsKeywords}}@endif" class="longinput"></span>
            </p>

            <p>
                <label>Nhập Tag</label>
                <span class="field"><input type="text" name="newstag" placeholder="Ví dụ : vụ-án, hình-sự, free-site, " value="@if(isset($objNews)){{$objNews->newsTag}}@endif" class="longinput"></span>
            </p>
            <p>
                <label>Link tới bài viết</label>
                <span class="field"><input type="text" name="newsSlug" placeholder="Ví dụ: pubweb.vn/tinngoaiweb/vu-an-hinh-su ; vu-an-hinh-su là từ được nhập" value='@if(isset($objNews)){{$objNews->newsSlug}}@endif' class="longinput"></span>
            </p>
