        ...
        jQuery('#deletepromulti').click(function() {
            ...
            if (addon != '') {
                jConfirm("{{Lang::get('messages.delete_confirm')}}", "{{Lang::get('messages.alert')}}", function(r) {
                    if (r == true) {
                        jQuery.post("{{URL::action('\BackEnd\HistoryUserController@postDelmulte')}}", {multiid: addon, user: user_id}).done(function(data) {
    ...
    <div class="subcontent">
        ...
        <table cellpadding="0" cellspacing="0" border="0" id="table2" class="stdtable stdtablecb">
            ...
            <tbody id="tableproduct">
                <?php $i = 1 ?>
                @include('backend.historyuser.HistoryUserajax')
            </tbody>
        </table>
