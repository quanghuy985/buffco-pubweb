...
class tblMenuModel extends \Eloquent {
    ...
    public function findMenuByID($id) {
        $objMenu = DB::table('tbl_menus')->where('id', '=', $id)->get();
        return $objMenu;
    }

    public function allMenu($per_page) {
        $arrMenu = DB::table('tbl_menus')->orderBy('id', 'desc')->paginate($per_page);
        return $arrMenu;
    }

    public function getMenu() {
        $arrMenu = DB::table('tbl_menus')->get();
        return $arrMenu;
    }
    ...
    public function countURL($url) {
        $objectMenu = DB::table('tbl_menus')->where('menuURL', 'LIKE', $url . '%')->count();
        return $objectMenu;
    }

    public function getAllMenuNew($start, $per_page) {
        $results = DB::select("select a.menuName as N1,a.menuName as menuName,a.menuParent as menuParent,a.menuName as menuParentName,a.id as id,a.menuURL as menuURL,a.menuPosition as menuPosition,a.time as time,a.status as status FROM `tbl_menus` as a where a.menuParent=0
        UNION ALL
        select a.menuName as N1,b.menuName as menuName,b.menuParent as menuParent,a.menuName as menuParentName,b.id as id,b.menuURL as menuURL,b.menuPosition as menuPosition ,b.time as time,b.status as status  FROM `tbl_menus` as a INNER JOIN `tbl_menus` as b On a.id = b.menuParent
