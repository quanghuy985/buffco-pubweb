...
<script type="text/javascript">
    function phantrang(page) {
        ...
        var urlpost = "{{URL::action('\BackEnd\OrderController@postAjaxOrder')}}?page=" + page
        if (jQuery('#datepicker').val() != '' && jQuery('#datepicker1').val() != '') {
            urlpost = "{{URL::action('\BackEnd\OrderController@postAjaxOrderFilter')}}?fromtime=" + jQuery('#datepicker').val() + "&totime=" + jQuery('#datepicker1').val() + "&status=" + jQuery('#status').val() + "&page=" + page;
        }
        if (jQuery('#searchblur').val() != '') {
            urlpost = "{{URL::action('\BackEnd\OrderController@postAjaxSearchOrder')}}?keyword=" + jQuery('#searchblur').val() + "&page=" + page;
        }
    ...
    function locdulieu() {
        ...
        jQuery.jGrowl("Đang tải dữ liệu ...");
        var request = jQuery.ajax({
            url: "{{URL::action('\BackEnd\OrderController@postAjaxOrderFilter')}}?fromtime=" + jQuery('#datepicker').val() + "&totime=" + jQuery('#datepicker1').val() + "&status=" + jQuery('#status').val(),
            type: "POST",
            dataType: "html"
        });
        request.done(function(msg) {
    ...
    function timkiem() {
        ...
        jQuery.jGrowl("Đang tải dữ liệu ...");
        var request = jQuery.ajax({
            url: "{{URL::action('\BackEnd\OrderController@postAjaxSearchOrder')}}?keyword=" + jQuery('#searchblur').val(),
            type: "POST",
            dataType: "html"
        });
        request.done(function(msg) {
    ...
    if (r == true) {
    var request = jQuery.ajax({
    url: "{{URL::action('\BackEnd\OrderController@postDel')}}?id=" + id,
            type: "POST",
            dataType: "html"
    });
            request.done(function(msg) {
        ...
        <table cellpadding="0" cellspacing="0" border="0"  class="stdtable stdtablecb">
            ...
            <tbody id="tableproduct">
                ...
                <tr>
                    ...
                    <td class="center">
                        <a href="{{URL::action('\BackEnd\OrderController@getEdit')}}/{{$item->orderCode}}" class="btn btn4 btn_orderdetail" title="Chi tiết đơn hàng"></a>
                        &nbsp; 
                        @if($item->status =='0')
